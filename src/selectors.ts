...
import { EnumsStore } from './enums-reducer';
...
export function getEnum(enumName: string): EnumValues {
  ...
  if (enumValues) {
    ...
    throw new MissingEnumException(
      `mad-spring-enum: The enum named '${enumName}' could not be found in the enum store, make sure the enums are loaded before the using them, and that the '${enumName}' actually exists.`,
    );
  }
}
