...
describe('Selectors', () => {
  ...
  describe('getEnum', () => {
    ...
    it('should throw an error when the enum cannot be found in the store', () => {
      expect(() => getEnum('NonExistent')).toThrow(`mad-spring-enum: The enum named 'NonExistent' could not be found in the enum store, make sure the enums are loaded before the using them, and that the 'NonExistent' actually exists.`);
    });
  });
});
